...
$sql = "SELECT class_sessions.*, courses.course, levels.level, blocks.block ,
    ...
    WHERE class_sessions.FacultyID = '$id' AND $blockID = class_sessions.BlockID
    AND class_sessions.FacultyID = faculty.ID AND class_sessions.BlockID = blocks.ID
    AND class_sessions.LabID = laboratories.ID
    AND courses.ID = levels.CourseID AND levels.ID = blocks.LevelID $filterbyMonth $filterbyYear ORDER BY `TimeIn` DESC";
...
try{
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        $classID = $row['ID'];
        $sql2 = "SELECT `sessions`.ID FROM `sessions`,`class_sessions` WHERE '$classID' = `sessions`.ClassID AND class_sessions.ID = `sessions`.ClassID";
        $noOfSessions = mysqli_num_rows(mysqli_query($conn, $sql2));
        $row['noOfSessions'] = (string) $noOfSessions;
        array_push($rows, $row);
    }
    $conn->close();
